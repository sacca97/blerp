...
cmd_gatt_write(int argc, char **argv)
{
    ...
    size = parse_arg_uint16("size", &rc);
    if (rc != 0) {
        console_printf("invalid 'size' parameter\n");
        return rc;
    }
    ...
    attr.om = ble_hs_mbuf_from_flat(data, size);
    ...
    if (no_rsp) {
        ...
        rc = btshell_write_no_rsp(conn_handle, attr.handle, attr.om);
        attr.om = NULL;
    }
    //  else if (is_long) {
    //     if (num_attrs != 1) {
